...
| **get_player_leaderboard** | User asks who had the **most wins**, **top performers**, or **best record** across **multiple past/in-progress events** in a region and date range (e.g. "who had the most wins in set championships in January 2026 in Detroit"). Single call; no need to search events then call get_event_standings per event. Date range max 3 months; radius max 100 miles. |
